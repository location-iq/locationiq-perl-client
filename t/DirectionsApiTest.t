...
#
{
    ...
    my $alternatives = undef; # replace NULL with a proper value
    ...
    my $continue_straight = undef; # replace NULL with a proper value
    my $result = $api->directions(coordinates => $coordinates, bearings => $bearings, radiuses => $radiuses, generate_hints => $generate_hints, approaches => $approaches, exclude => $exclude, alternatives => $alternatives, steps => $steps, annotations => $annotations, geometries => $geometries, overview => $overview, continue_straight => $continue_straight);
}
